...
router.route('/create').post(async (req, res) => {
    try {
        ...
        if (!name || !game_id ||!amount) {
            return res.status(400).json({ message: "Please fill up all the required details." });
        }
...
router.route('/lap:id').post(async(req,res)=>{
    ...
    user.findOneAndUpdate({_id:id},{$set:{amount:player_Amount}},{new: true},(err,data)=>{
        if(err){
...
router.route('/transfer/:id').post(async (req, res) => {
    try {
        ...
        if (!id || !player_id || !amountSend) {
            return res.status(400).json({ message: "ID, player_id, and amountSend are required fields." });
        }
        ...
        if (!receiverPlayer || !senderPlayer) {
            return res.status(404).json({ message: "Receiver or sender player not found." });
        }
...
router.route('/properties_Buying:id').post(async(req,res)=>{
    ...
    // })
        ...
        const updated_Properties=[...current_Player.properties,property.toUpperCase()]

        await user.findOneAndUpdate({_id:id},{$set:{properties:updated_Properties}},{new: true},(err,data)=>{
            if(err){
                throw err
            }
...
router.route('/pay_Bank:id').post(async(req,res)=>{
    ...
    const player_Updated_Amount = current_player.amount - (pay_Bank)
            ...
            user.findOneAndUpdate({_id:player_id},{$set:{amount:player_Updated_Amount}},{new: true},(err,data)=>{
            if(err){
...
router.route('/adding_Credits:id').post(async(req,res)=>{
    ...
    const updated_Amount= player_Details.amount + Number(amount)
     ...
     user.findOneAndUpdate({_id:id},{$set:{amount:updated_Amount}},{new: true},(err,data)=>{
        if(err){
            throw new err
        }
